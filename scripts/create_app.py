...
output_file = open("create-app.sh", "w")

template = (
    "argocd app create qa-7-{} --repo {} --revision "
    "sc-220063_ "
    "--project inc-qa-7 --path deploy/kube-manifests-qa-argo "
    "--dest-namespace inc-qa --dest-server "
    "https://9E1A49378D4015F77A195C5757B1E538.gr7.us-east-1.eks.amazonaws.com --directory-recurse\n"
)

for repo in repos:
    full_repo = repo
    repo_name = repo.split("/")[-1].replace(".git", "")
    output_file.write(template.format(repo_name, full_repo))
